...
| **4 — Contracts + Tests + Testnet Deploy** | Foundry project: all six contracts + EIP-712 witness lib + genesis-attestor; full test suite; deploy/configure/seed scripts; **deployed & verified on Base Sepolia**. | Coverage gate met; all invariants pass (soulbound, one-vote, no-double-claim, treasury non-negative, reward solvency); slither/solhint clean or triaged; contracts verified on Base Sepolia; fork tests green against live addresses. |
