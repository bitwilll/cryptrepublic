...
### 5.11 Wallet modes (Wave 11 — delivered)

The wallet screen is a **mode chooser** with three non-custodial modes, persisted as public metadata in the wallet IndexedDB (`meta` store, DB v2): **embedded** (create or IMPORT a BIP-39 vault — validate-first, explicit confirmed overwrite, phrase never echoed), **hardware/external** (wagmi `injected()` + WalletConnect; plain sends via the wallet's own signer — `sendEvmExternal`; correct-chain guard; direct Ledger WebHID remains a documented deferral), and **watch-only + air-gapped** (a checksum-validated public address drives the read-only portfolio under a WATCH-ONLY badge; sends are QR round-trips: the watch-only device builds an UNSIGNED EIP-1559 tx in a versioned self-contained envelope — `cr-eth-tx-unsigned`, single QR, EC-L capacity 2953 bytes guarded before render, BC-UR multi-part as follow-up — the offline signer (the embedded wallet's "scan a request to sign" mode) decodes it HONESTLY (ERC-20 recipient+amount decoded from transfer calldata, token contract surfaced; never the raw token-contract `to`/`0` value), signs locally with `account.signTransaction`, never broadcasts, and returns a signed QR the watch-only device scans and relays via the allow-listed `eth_sendRawTransaction` proxy to a receipt-gated "sent"). The camera scanner is bundled pure-JS `jsqr` (no CDN/WASM/worker → zero CSP change), tap-to-scan with a manual-paste fallback and full track cleanup. **Non-custodial invariants:** the watch-only path holds no key and never imports the signer module graph (transitive static guard); the offline signer makes zero network calls; `test/no-secret-to-fetch.test.ts` scans fetch bodies AND QR payloads across the loop; an anvil integration proof drives the whole round-trip with only a throwaway test key signing. `POST /api/wallet/link` (SIWE-proven) binds a verified wallet to the logged-in account. Watch-only/air-gapped is EVM-only in this wave (Solana/BTC follow-up).
...
Nine cumulative waves; each lists its deliverable and the acceptance criteria that make it "done." Regression suites from prior waves stay green. (Wave 9, the Admin Panel, was added 2026-07-01 as a post-app capstone.)
...
| **9 — Admin Panel** (capstone; added 2026-07-01) | Admin-only, role-gated back office with an audit log: user/citizen management (list/search, sessions, suspend, KYC status); citizenship-application review; content management for the DB-served content (embassies directory, sovereign-holdings/asset catalog, governance proposals, population/census); feature flags; and PREPARING on-chain admin actions (role grants, dividend epochs, treasury proposals, pause) for the user's Safe/multisig to sign. | Admin role + guarded routes enforced (non-admins 403/redirect); every mutation authorized + audit-logged; user/application/content CRUD works and is covered by tests; **non-custodial** — the panel manages off-chain data and PREPARES on-chain admin txs but NEVER holds keys, signs, or moves funds; full app+contract suites stay green. |
